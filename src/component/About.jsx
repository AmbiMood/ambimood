...
const About = () => {
  ...
  const styles = {
    ...
    brandName: { fontSize: '1.1rem', fontWeight: '800', color: 'white', letterSpacing: '3px', fontFamily: 'serif', textDecoration: 'none' },
    brandAccent: { color: '#a78bfa' },
    content: { maxWidth: '800px', margin: '0 auto', padding: '110px 20px 80px', position: 'relative', zIndex: 1 },
    ...
    title: { fontSize: 'clamp(2rem, 8vw, 4rem)', fontWeight: '800', color: 'white', marginBottom: '24px', lineHeight: '1.1', fontFamily: 'serif', letterSpacing: '-1px' },
    ...
    description: { fontSize: 'clamp(0.9rem, 3vw, 1.1rem)', color: 'rgba(255,255,255,0.6)', lineHeight: '1.8', marginBottom: '20px', fontWeight: '300' },
    cardsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px', marginTop: '48px' },
    card: { background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '20px', padding: '24px 20px', backdropFilter: 'blur(10px)' },
    cardEmoji: { fontSize: '2rem', marginBottom: '12px', display: 'block' },
    cardTitle: { fontSize: '0.95rem', fontWeight: '700', color: 'white', marginBottom: '8px' },
    cardDesc: { fontSize: '0.82rem', color: 'rgba(255,255,255,0.45)', lineHeight: '1.6' },
  };
    ...
    <div style={styles.page}>
      ...
      <nav style={styles.navbar}>
        ...
        <div className="desktop-nav" style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
          {navLinks.map(l => (
            <Link key={l.label} to={l.href} style={{
              color: l.href === '/about' ? 'white' : 'rgba(255,255,255,0.6)',
              textDecoration: 'none', fontSize: '0.85rem', fontWeight: '500',
              padding: '7px 14px', borderRadius: '100px',
              background: l.href === '/about' ? 'rgba(139,92,246,0.2)' : 'transparent',
              border: l.href === '/about' ? '1px solid rgba(139,92,246,0.3)' : '1px solid transparent',
            }}>{l.label}</Link>
          ))}
        </div>
        ...
        <button className="hamburger-btn" onClick={() => setMenuOpen(!menuOpen)} style={{
          display: 'none', background: 'none', border: '1px solid rgba(139,92,246,0.3)',
          borderRadius: '8px', color: '#c4b5fd', fontSize: '1.2rem', cursor: 'pointer', padding: '4px 10px',
        }}>{menuOpen ? '✕' : '☰'}</button>
      </nav>
      ...
      {menuOpen && (
        <div style={{ position: 'fixed', top: '54px', left: 0, right: 0, zIndex: 99, background: 'rgba(10,10,15,0.97)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(139,92,246,0.2)', padding: '8px 16px 16px' }}>
          {navLinks.map(l => (
            <Link key={l.label} to={l.href} onClick={() => setMenuOpen(false)} style={{ display: 'block', color: 'rgba(196,181,253,0.85)', textDecoration: 'none', fontSize: '1rem', fontWeight: '600', padding: '13px 12px', borderBottom: '1px solid rgba(139,92,246,0.08)' }}>{l.label}</Link>
          ))}
        </div>
      )}
      ...
      {/* Content */}
      <div style={styles.content}>
        <div style={styles.badge}><span style={styles.dot}></span>About Us</div>
        <h1 style={styles.title}>About <span style={styles.titleAccent}>AMBI MOOD</span></h1>
        <div style={styles.divider} />
        <p style={styles.description}>AMBI MOOD is an innovative music recommendation app that uses emotion detection to suggest the perfect songs for your current mood.</p>
