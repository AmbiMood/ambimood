...
//         `https://ambimood-backend-2.onrender.com/api/music/recommend?emotion=${detectedEmotion}&language=${language}`
//       );
...
//     //calm: 'linear-gradient(135deg, #0a1628 0%, #0d2137 50%, #0a2e2e 100%)',
//     //romantic: 'linear-gradient(135deg, #1a0a1a 0%, #2d1530 50%, #1a0d2e 100%)',
//     //energetic: 'linear-gradient(135deg, #1a0a00 0%, #2d1f00 50%, #1a1500 100%)',
...
//           {modelsLoaded ? '✅ Ready To— Detect your emotion!' : '⏳ Loading AI models...'}
//         </p>
//         <p style={{ ...styles.subtitle, fontSize: '0.85rem', marginTop: '-36px' }}>
...
//                       const res = await fetch('https://ambimood-backend-2.onrender.com/api/save-favorite', {
//                         method: 'POST',
...
//                       //   if (data.message === 'Saved!') alert('❤️ Saved to Favorites!');
//                       // }}
//                       // style={{
//                       //   padding: '8px 12px',
...
//     if (!file) return;
//     const reader = new FileReader();
//     reader.onloadend = async () => { setUploadedImage(reader.result); await analyzeEmotionFromBase64(reader.result); };
//     reader.readAsDataURL(file);
//   };
...
//     try {
//       const img = document.createElement('img');
//       img.src = base64Image; img.style.display = 'none';
//       document.body.appendChild(img);
//       await new Promise((resolve, reject) => { img.onload = resolve; img.onerror = reject; });
//       const detections = await faceapi.detectAllFaces(img, new faceapi.TinyFaceDetectorOptions()).withFaceExpressions();
//       document.body.removeChild(img);
//       if (!detections || detections.length === 0) { setDetectedEmotion('No face found! Try again 📸'); setLoading(false); return; }
//       const expressions = detections[0].expressions;
//       const emotion = Object.keys(expressions).reduce((a, b) => expressions[a] > expressions[b] ? a : b);
//       setDetectedEmotion(emotion);
//       setHeartRate(calculateHeartRate(emotion));
//       setShowLanguageSelect(true);
//     } catch (error) { setDetectedEmotion('Detection failed — try another photo'); }
...
//       <div style={{ maxWidth: '800px', margin: '0 auto', padding: '80px 16px 60px', position: 'relative', zIndex: 1 }}>
...
//         </h1>
//         <p style={{ color: 'rgba(255,255,255,0.45)', marginBottom: '6px', fontSize: '0.95rem', fontWeight: '300' }}>
//           {modelsLoaded ? '✅ Ready To— Detect your emotion!' : '⏳ Loading AI models...'}
//         </p>
...
//               <button className="btn-camera" onClick={() => setShowCamera(true)}>📷 Open Camera</button>
//               <button className="btn-upload" onClick={() => fileInputRef.current.click()}>🖼️ Upload Image</button>
...
//           {showCamera && (
//             <div>
//               <Webcam ref={webcamRef} audio={false} screenshotFormat="image/jpeg" style={{ width: '100%', maxWidth: '480px', borderRadius: '16px', border: '2px solid rgba(139,92,246,0.4)', display: 'block', margin: '0 auto' }} />
//               <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '16px', flexWrap: 'wrap' }}>
//                 <button onClick={capturePhoto} style={{ padding: '11px 24px', background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.4)', borderRadius: '100px', color: '#86efac', fontSize: '0.9rem', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>📸 Capture</button>
//                 <button onClick={() => setShowCamera(false)} style={{ padding: '11px 24px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '100px', color: '#fca5a5', fontSize: '0.9rem', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>✕ Cancel</button>
//               </div>
//             </div>
//           )}
...
//               <img src={capturedImage || uploadedImage} alt="Face" style={{ maxWidth: '240px', width: '100%', borderRadius: '16px', border: '2px solid rgba(139,92,246,0.3)', marginBottom: '16px' }} />
...
//                 </div>
//               )}
//               <button onClick={() => { setCapturedImage(null); setUploadedImage(null); setDetectedEmotion(''); setSongs([]); setShowLanguageSelect(false); setSelectedLanguage(''); setHeartRate(null); }}
//                 style={{ padding: '10px 24px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '100px', color: '#fca5a5', fontSize: '0.88rem', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>↺ Try Again</button>
//             </div>
//           )}
//         </div>
