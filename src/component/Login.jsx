  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        if (retry.ok) {
          localStorage.setItem('userName', retryData.user?.name || formData.email.split('@')[0]);
          localStorage.setItem('userEmail', retryData.user?.email || formData.email);
          navigate('/');
          return;
        } else {
          setError(retryData.message || 'Invalid email or password!');
        }
      } catch (e) {
        setError('Cannot connect to server! Try again.');
      }
